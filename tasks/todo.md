...
- [x] Figure 1 cribbed at native resolution (2250×2400) via embedded-image extraction (better than the planned render-crop) → `public/papers/wfh-bank-efficiency/fig1-dynamic-effects.png`; reusable `scripts/extract_figures.py` committed (embedded-image + render-crop + `--probe` modes)
- [x] `papers/wfh-bank-efficiency.json`: six prose sections (intro w/ dropcap, design, results, dynamics, mechanisms, inverse impact w/ pull-quote), `coefficient-bar` from Table 3 cols. 4 & 8 (REAL printed values, incl. p-values: cost −0.086*/+0.101**, revenue +0.027/−0.095**, profit −0.012/+0.032), static Fig 1, concept-map mechanism (6 nodes/6 edges incl. dashed moderators), abstract-network hero, Fed disclaimer in footer, SSRN link
- [x] Used `coefficient-bar` instead of the planned `grouped-bar` for Table 3 — the estimates are signed; `grouped-bar` assumes non-negative magnitudes
- [ ] Interactive event-study rebuild of Fig 1: BLOCKED on replication output from David (coefficients + CI bounds per horizon × 4 panels) — deliberately omitted rather than shipped "stylized", since the static crib already shows the real figure on the same page (numbers policy)
- [x] Fixed `ProseSection` tone/layout coupling found during pilot verification: `tone: 'inverse'` now works in ALL layouts (shared `INVERSE_SECTION_CLASS` + `InverseGlow`; `QuoteBlock` gained an `inverse` variant). Previously only `figure-left` had a dark branch — the pilot's `prose`+`inverse` section rendered inverse text colors on a light background. AlphaQubit regression-checked after (its `figure-left`+`inverse` innovation section renders identically).
- [x] Browser-verified the full pilot page (all sections, both figure cards, concept map incl. legend, dark impact section, authors, footer disclaimer) + gallery now shows both papers (`_preview` still hidden)
