...
paperViz is a reusable engine, originally exported from Google AI Studio as a one-off AlphaQubit showcase, that turns a `papers/<slug>.json` content file into a themable, interactive marketing site for a research paper. It supports three figure classes — interactive templated components, static images cribbed from the paper PDF, and LLM-generated concept-map diagrams from a custom SVG spec — and deploys to GitHub Pages for embedding via iframe into David's Google Sites research hub. Content and figure generation happen at authoring time inside Codex sessions; the shipped app makes no LLM calls at runtime.
