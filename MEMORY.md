...
- **[2026-07-30] Second real paper onboarded: `dark-banking` ("Dark Banking? Banks and Illicit Deposit Flows", David's solo paper, SSRN 3811752, March 2021).** Source PDF `C:/Users/david/OneDrive/Escritorio/ssrn_id3811752_code2599389.pdf` (63 pp.) — the Read tool falsely reports it password-protected; PyMuPDF opens it fine (`needs_pass: 0`), so all extraction goes through the anaconda interpreter + fitz. Exhibits: `coefficient-bar` from Table 2 Panel A printed values, static Fig 1 (cartel-expansion maps, native embedded image) + Fig 3 (event-time panels, two embedded images stacked with PIL), `event-study-explorer` from Table 6 col (2) printed values, and a 7-node concept map.
